(function () {
  ...
  const addTodo = (val, created, itemID, completedOn = false) => {
    ...
    editConfirmListener(
      todo.getElementsByClassName("todo-item-edit-confirm")[0]
    );
  ...
  const completeListener = (btn) => {
    ...
    btn.addEventListener("click", () => {
      if (isCompleted) {
        item.remove();
        addTodo(
          item.getElementsByClassName("todo-item-content")[0].innerHTML,
          item.dataset.createdOn,
          item.id.split("-")[1]
        );
        setListIDs();
      } else {
  ...
  const editFieldListener = (field) => {
    ...
    field.addEventListener("keyup", (e) => {
      if (e.keyCode === 13) {
        ...
        wrap.getElementsByClassName("todo-item-content")[0].innerHTML =
          field.value;
        wrap
          .getElementsByClassName("todo-item-content")[0]
          .classList.remove("inactive");
        wrap
          .getElementsByClassName("todo-item-content-edit")[0]
  ...
  const editListener = (btn) => {
    ...
    btn.addEventListener("click", () => {
      ...
      wrap.getElementsByClassName("todo-item-edit-field")[0].value =
        wrap.getElementsByClassName("todo-item-content")[0].innerHTML;
      wrap
        .getElementsByClassName("todo-item-content-edit")[0]
  ...
  const editCancelListener = (btn) => {
    ...
    btn.addEventListener("click", () => {
      ...
      wrap
        ...
        .classList.remove("active");
      wrap
        .getElementsByClassName("todo-item-edit")[0]
  ...
  const deleteListener = (btn) => {
    ...
    let wrap = btn.parentNode.parentNode;
    btn.addEventListener("click", () => {
      item.getElementsByClassName("todo-item-deleted")[0].height =
        item.clientHeight;
      item
        .getElementsByClassName("todo-item-deleted")[0]
        .classList.add("active");
      wrap.classList.add("slide");
      setTimeout(() => {
        item.remove();
    ...
    if (incompleteTodos.length > 0) {
      for (let i = 0; i < incompleteTodos.length; i++) {
        ...
        listItems.incomplete[i + 1] = {
          todo: incompleteTodos[i].getElementsByClassName(
            "todo-item-content"
          )[0].innerHTML,
    ...
    if (completeTodos.length > 0) {
      for (let i = 0; i < completeTodos.length; i++) {
        ...
        listItems.complete[i + 1] = {
          todo: completeTodos[i].getElementsByClassName("todo-item-content")[0]
            .innerHTML,
    ...
    inputField.addEventListener("keyup", (e) => {
      if (e.keyCode === 13) {
        ...
        addTodo(
          inputField.value,
          moment().format("YYYY-MM-DD"),
          document.getElementsByClassName("todo-item").length + 1
        );
      }
    });
